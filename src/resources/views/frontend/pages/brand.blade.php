...
@section('meta')
<!-- Title -->
<title>AB Electronics - {{ $brand->name }} Brand</title>
@endsection
    ...
    <div class="container">
        <div class="row">
            <div class="banner-carousel owl-carousel owl-theme">
                @foreach( $brand->banner as $banner )
                <div class="item">
                    <div class="col-md-12">
                        <img src="{{ asset('images/brand/'. $banner->image) }}" style="width: 100%;" class="img-fluid"
                            alt="">
                    </div>
                </div>
                @endforeach
            ...
            <div class="col-md-3 full-col" style="padding:0;border: 1px solid #f3f1f1;">
                <div class="left cat-left">
                    ...
                    <div class="category-filter-block">
                        ...
                        <!-- category start -->
                        @if( $brand->category->where("is_active", true)->count() > 0 )
                        <div class="manufacturer" id="cat">
                            <h2>Category</h2>
                            <ul>
                                @foreach( $brand->category->where("is_active", true) as $category )
                                <li id="{{ $category->id }}">
                                    {{ $category->name }}
                                </li>
                                @endforeach
                        ...
                        <!-- sub category start -->
                        @if( $brand->subcategory->where("is_active", true)->count() > 0 )
                        <div class="manufacturer" id="sub_cat">
                            <h2>Sub Category</h2>
                            <ul>
                                @foreach( $brand->subcategory->where("is_active", true) as $subcategory )
        ...
        <div class="row type-wise-row">
            ...
            <div class="col-md-2 col-6" style="padding: 0">
                <div class="product-box">
                    ...
                    <a class="product_name">{{ Str::limit($product->name, 35, '...') }}</a>
                    <div class="row">
                        <div class="col-md-6 col-6">
                            @if ($product->offer_price == null)
                            <p class="offer_price" style="text-align: left">
...
<script>
    ...
    $("#product-row .right .left ul li").click(function(){
        ...
        //category check
        if( $("#cat ul .active-cat").attr("id") ){
            cat = $("#cat ul .active-cat").attr("id")
        }
        else{
            cat = null
        }
    ...
    $(".price-range").on('click','.noUi-origin', function(){
        ...
        if (min_value < 0 || max_value < 0) {
            ...
            $.ajax({
                type: "GET",
                url: "/min/" + min_value + "/max/" + max_value + "/brand/" + brand_id,
                contentType: false,
                processData: false,
                cache: false,
                success: function(response) {
    ...
    $("#cat ul li").click(function() {
        ...
        const $this = $(this)
        $("#cat ul li").removeClass("active-cat")
        ...
        if (category && category > 0) {
            ...
            $(".loader").hide()
            $("#cat .failed").show()
            $("#cat .failed").html('Invalid category')
        }
    ...
    $("#sub_cat ul li").click(function() {
        ...
        if (sub_category && sub_category > 0) {
            $.ajax({
                type: "GET",
                url: "/sub_cat/"+sub_category+"/brand/"+brand,
                contentType: false,
                processData: false,
                cache: false,
                success: function(response) {
