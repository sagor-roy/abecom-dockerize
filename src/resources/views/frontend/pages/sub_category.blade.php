...
@section('meta')
<!-- Title -->
<title>AB Electronics - {{ $sub_cat->name }}</title>
@endsection
                        ...
                        <ul>
                            ...
                            <li id="{{ $category->id }}">
                                ...
                                <div class="category-dropdown {{ $category->id }}">
                                    <ul>
                                        @foreach ($category->subcategory->where("is_active", true) as $sub_category)
                                        <li>
                                            <a
                                                href="{{ route('subcategory', $sub_category->slug) }}">{{ $sub_category->name }}</a>
                                        </li>
                                        @endforeach
                                    </ul>
                                </div>
    ...
    <div class="container">
        <div class="row">
            <ul>
                ...
                <li>
                    <a href="{{ route('category', $sub_cat->category->slug) }}">
    ...
    <div class="container">
        <div class="row">
            <div class="banner-carousel owl-carousel owl-theme">
                @foreach( $sub_cat->banner as $banner )
                <div class="item">
                    <div class="col-md-12">
                        <img src="{{ asset('images/subcategory/'. $banner->image) }}" style="width: 100%;"
                            class="img-fluid" alt="">
                    </div>
                </div>
                @endforeach
        ...
        <div class="row">
            ...
            <div class="col-md-3 full-col" style="padding:0;border: 1px solid #f3f1f1;">
                <div class="left cat-left">
                    ...
                    <!-- brand start -->
                    @if( $sub_cat->category->brand->where("is_active", true)->count() > 0 )
                    <div class="manufacturer" id="brand">
                        <h2>Brands</h2>
                        <ul>
                            @foreach( $sub_cat->category->brand->where("is_active", true) as $brand )
                            <li id="{{ $brand->id }}">
                                {{ $brand->name }}
                            </li>
                            @endforeach
                    ...
                    @if( $varient->category_varient->where('category_id', $sub_cat->category->id)->where("is_active", true)->count() > 0 )
                    <div class="manufacturer category_varient">
                        <h2>{{ $varient->name }}</h2>
                        <ul>
                            @foreach( $varient->category_varient->where('category_id', $sub_cat->category->id)->where("is_active", true) as
                            $category_varient )
...
<script>
    ...
    $(".price-range").on('click','.noUi-origin', function(){
        ...
        if (min_value < 0 || max_value < 0) {
            ...
            $.ajax({
                type: "GET",
                url: "/sub/cat/"+sub_cat_id+"/min/"+min_value+"/max/"+max_value,
                contentType: false,
                processData: false,
                cache: false,
                success: function(response) {
    ...
    $("#brand ul li").click(function() {
        ...
        if (id && id > 0) {
            $.ajax({
                type: "GET",
                url: "/brand/" + id + "/subcat/" + sub_cat_id,
                contentType: false,
                processData: false,
                cache: false,
                success: function(response) {
    ...
    //sub category wise product varient code start
    $(".category_varient ul li").click(function(){
        ...
        if( id < 1 ){
            ...
            $.ajax({
                type: "GET",
                url: "/category_attribute/" + id + "/subcat/"+sub_cat_id,
                contentType: false,
                processData: false,
                cache: false,
                success: function(response) {
